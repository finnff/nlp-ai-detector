...
\subsubsection{Classification Metrics} All classifiers are evaluated on the held-out test set (20\% of data) using multiple performance metrics: accuracy, precision, recall, F1-score (macro, weighted, and per-class), AUROC (Area Under the ROC Curve). For the ensemble, both individual model performance and complete ensemble performance are reported, to allow comparison between the internal ensemble methods and the overall ensemble performance.
