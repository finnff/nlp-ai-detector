...
The ensemble-based detector aims to build a practical, robust detector by combining multiple classifiers via majority voting. The ensemble includes: a BERT classifier; a Naive Bayes classifier; potentially a feature-based model (TODO update after discussion!!). The ensemble works by letting each model making a prediction, then "voting", selecting the majority class. This approach increases robustness against adversarial attacks and domain shifts, as fooling the ensemble would require deceiving a majority of its components simultaneously.
% TODO this need to be refined. And probably there will be weighting involved??
