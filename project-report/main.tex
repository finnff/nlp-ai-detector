...
To reduce collinearity, a correlation analysis was performed on all the described extracted features, and from pairs with high pairwise correlation ($r > 0.8$), one feature was dropped. The feature to be dropped was selected based on whether it was correlated with more other features, and which feature would be more informative in the project context. After this selection process, 40 linguistic and statistical features were left for model training. The full list of selected and dropped features is shown in Table \ref{tab:feature-selection}.
...
Table \ref{tab:feature-based-results} presents the performance of the three feature-based classifiers on the test set (22,570 samples, 11,285 per class). XGBoost achieved the highest performance with 97.45\% accuracy, followed by Random Forest (96.06\%) and Logistic Regression (92.82\%).
...
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Metric} & \textbf{Logistic Regression} & \textbf{Random Forest} & \textbf{XGBoost} \\
\midrule
...
Figures \ref{fig:lr-features}, \ref{fig:rf-features}, and \ref{fig:xgb-features} show the top 10 most important features for each of the three feature-based classifiers. Across all three models, perplexity is the most discriminative feature, with normalized importance scores ranging from 0.334 (XGBoost) to 0.430 (Random Forest). Lexical features, particularly average word length and hapax legomena ratio, consistently rank among the top features. Syntactic features show varying importance across models, with Random FOrest and Logistic Regression having four syntactic features among their top 10, and XGBoost only one, and parse depth variance being the second most important feature for Logistic Regression (0.179) but not appearing at all in the top 10 for XGBoost.
