...
The datasets showed varying degrees of class imbalance between the target classes human-written (0) and AI-generated (1), ranging from 31.5\% AI-generated in the HC3 dataset to 50.0\% AI-generated in the AH\&AITD dataset. The dataset's class distribution is shown in Table \ref{tab:dataset-class-distribution}.
...
% Note: DeBERTa performance may differ on the full 112k dataset as these results reflect initial experiments. 
...
The ensemble methods demonstrated robust performance across multiple evaluation metrics, with voting and averaging approaches providing consistent improvements over individual classifiers in terms of stability and generalization.

Having examined both feature-based results and ensemble performance, we now discuss the key findings and their implications for AI text detection.
...
Building on these feature-level insights, we now examine how ensemble-based approaches compare to individual classifiers in terms of detection performance.
...
A limitation of this project is that our datasets include only samples that are either entirely human-written or entirely AI-generated, with no mixed or (adversarially) modified samples. In real-world scenarios, users may use different techniques to fool the detectors and evade detection. Our models were not trained or evaluated on such samples and their robustness to these scenarios would require further evaluation before practical deployment. 
