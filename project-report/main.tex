...
The availability of large language models (LLMs) has created an urgent need for reliable detection of AI-generated text in domains such as academia and journalism. Recent surveys indicate that hybrid approaches to AI detection, combining statistical and neural methods, often outperform single-modality detectors in both robustness and generalization \citep{wu-etal-2025-survey, su2024robustdetectionllmgeneratedtext}.

Motivated by these findings, this project takes a dual approach to AI text detection. First, we conduct a systematic feature-based analysis to identify which linguistic and statistical patterns are most discriminative. Second, we implement an ensemble-based detector combining multiple classification approaches to explore whether such combinations can improve practical deployment robustness.
...
\begin{itemize}[noitemsep,nolistsep]
    ...
    \item \textbf{Random Forest}: Default scikit-learn parameters, max\_depth 10, random state 42
    \item \textbf{XGBoost (feature-based)}: 100 estimators, max depth 6, learning rate 0.1, random state 42, eval\_metric 'logloss'
\end{itemize}
...
\paragraph{Classification Performance}
Table \ref{tab:ensemble-component-results} presents the performance of all classifiers in the ensemble-based detector. The feature-based XGBoost classifier achieved the highest performance (97.6\% accuracy, 0.997 AUROC). Among the text-based classifiers, DeBERTa performed best (94.2\% accuracy, 0.988 AUROC), followed by BERT (92.1\% accuracy, 0.975 AUROC).
...
\section{Full List of Features Extracted}
\label{sec:full-list-of-features}
\texttt{['average_word_length', 'word_length_variance', 'rttr_root_type_token_ratio', 'hapax_legomena_ratio', 'average_sentence_length', 'sentence_length_variance', 'average_parse_depth', 'maximum_parse_depth', 'parse_depth_variance', 'comma_ratio', 'period_ratio', 'exclamation_mark_ratio', 'question_mark_ratio', 'punctuation_per_word', 'pos_ratio_adj', 'pos_ratio_adp', 'pos_ratio_adv', 'pos_ratio_aux', 'pos_ratio_conj', 'pos_ratio_cconj', 'pos_ratio_det', 'pos_ratio_intj', 'pos_ratio_noun', 'pos_ratio_num', 'pos_ratio_part', 'pos_ratio_pron', 'pos_ratio_propn', 'pos_ratio_punct', 'pos_ratio_sconj', 'pos_ratio_sym', 'pos_ratio_verb', 'pos_ratio_x', 'noun_to_verb_ratio', 'sentence_length_burstiness', 'sentence_length_iqr', 'position_burstiness_diff', 'long_sentence_ratio', 'word_length_burstiness', 'clause_length_burstiness', 'generated', 'perplexity']}
