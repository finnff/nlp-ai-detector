...
class PerplexityProcessor:
    ...
    def process_dataset(self, input_path: str, output_path: str, text_column: str = "text", file_type: str = "csv", source_name: str = None, **kwargs) -> bool:
        ...
        # Load dataset - Special handling for EvoBench JSON datasets first
        if file_type == 'evobench_json':
            import json

            # Load configuration from kwargs
            model_families = kwargs.get('model_families', [])
            domains_to_include = kwargs.get('domains_to_include', [])
        ...
        elif file_type == 'arrow' or input_path.endswith('.arrow'):
...
def main():
    ...
    for source_name, config in sources_to_process.items():
        ...
        try:
            # Prepare additional parameters for specific source types
            extra_params = {}
            if source_name == 'evobench':
                extra_params['model_families'] = config.get('model_families', [])
                extra_params['domains_to_include'] = config.get('domains_to_include', [])
